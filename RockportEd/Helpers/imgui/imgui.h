...
// Structure used to constraint window size in custom ways when using custom ImGuiSizeCallback (rare/advanced use)
...
namespace ImGui
{
    ...
    // start a new ImGui frame, you can submit any command from this point until Render()/EndFrame().
    IMGUI_API void Render();
    // ends the ImGui frame, finalize the draw data. (Obsolete: optionally call io.RenderDrawListsFn if set. Nowadays, prefer calling your render function yourself.)
    IMGUI_API ImDrawData* GetDrawData();
    // valid after Render() and until the next call to NewFrame(). this is what you have to render. (Obsolete: this used to be passed to your io.RenderDrawListsFn() function.)
    IMGUI_API void EndFrame();
    // ends the ImGui frame. automatically called by Render(), so most likely don't need to ever call that yourself directly. If you don't need to render you may call EndFrame() but you'll have wasted CPU already. If you don't need to render, better to not create any imgui windows instead!
    ...
    // get draw list associated to the window, to append your own drawing primitives
    ...
    // retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwhise use GetColorU32() to get style color with style alpha baked in.
    ...
    IMGUI_API bool InputDouble(const char* label, double* v, double step = 0.0f, double step_fast = 0.0f,
                               const char* display_format = "%.6f", ImGuiInputTextFlags extra_flags = 0);
    ...
    IMGUI_API bool
    BeginPopupContextWindow(const char* str_id = NULL, int mouse_button = 1, bool also_over_items = true);
...
enum ImGuiBackendFlags_
{
    ...
    // Back-end can honor io.WantSetMousePos and reposition the mouse (only used if ImGuiConfigFlags_NavEnableSetMousePos is set).
};
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags__OptionsDefault = ImGuiColorEditFlags_Uint8 | ImGuiColorEditFlags_RGB |
...
struct ImGuiIO
{
    ...
    // = 0                  // Set ImGuiBackendFlags_ enum. Set by imgui_impl_xxx files or custom back-end.
    ...
    ImFontAtlas* Fonts;
    // <auto>               // Load and assemble one or more fonts into a single tightly packed texture. Output to Fonts array.
    ...
    // <unset> (0.0f,0.0f)  // If you use DisplaySize as a virtual space larger than your screen, set DisplayVisibleMin/Max to the visible area.
    ImVec2 DisplayVisibleMax;
    // <unset> (0.0f,0.0f)  // If the values are the same, we defaults to Min=(0.0f) and Max=DisplaySize
    ...
    bool MouseDown[5];
    // Mouse buttons: left, right, middle + extras. ImGui itself mostly only uses left button (BeginPopupContext** are using right button). Others buttons allows us to track if the mouse is being used by your application + available to user as a convenience via IsMouse** API.
    ...
    // Track if button was clicked inside a window. We don't request mouse capture from the application if click started outside ImGui bounds.
...
struct ImColor
{
    ...
    unsigned int save_minimal(Archive&) const
    {
    ...
    template <class Archive,
              cereal::traits::EnableIf<cereal::traits::is_text_archive<Archive>::value>  = cereal::traits::sfinae>
    void load_minimal(Archive const&, unsigned int const& imcol)
    {
        float sc = 1.0f / 255.0f;
        Value.x = (float)((imcol >> IM_COL32_R_SHIFT) & 0xFF) * sc;
        Value.y = (float)((imcol >> IM_COL32_G_SHIFT) & 0xFF) * sc;
        Value.z = (float)((imcol >> IM_COL32_B_SHIFT) & 0xFF) * sc;
        Value.w = (float)((imcol >> IM_COL32_A_SHIFT) & 0xFF) * sc;
    }
};
...
struct ImDrawList
{
    ...
    IMGUI_API void
    PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);
    ...
    IMGUI_API void PathStrokeGradient(ImVec2* gradient_p, ImU32* col, bool closed, float thickness = 1.0f);
    ...
    IMGUI_API void PathArcToDashedAndStroke(const ImVec2& centre, float radius, float a_min, float a_max, ImU32 col,
                                            float thickness = 1.0f, int num_segments = 10, int seq_draw_segments = 1,
                                            int seq_ignore_segments = 1, int num_start_segment = 0,
                                            int num_last_segment = -1);
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont* AddFontFromMemoryTTF(void* font_data, int font_size, float size_pixels,
                                           ...
                                           const ImWchar* glyph_ranges = NULL);
    // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after Build(). Set font_cfg->FontDataOwnedByAtlas to false to keep ownership.
    IMGUI_API ImFont* AddFontFromMemoryCompressedTTF(const void* compressed_font_data, int compressed_font_size,
...
struct ImFont
{
    ...
    float Scale;
    // = 1.f        // Base font scale, multiplied by the per-window font scale which you can adjust with SetFontScale()
    ...
    // ~ 1          // Number of ImFontConfig involved in creating this font. Bigger than 1 when merging multiple font sources into one ImFont.
