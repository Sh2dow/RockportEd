    ...
    namespace InGameMenu
    {
        ...
        public:
                        ...
                        if (CameraInternals::getActiveCameraInfo(pActiveCameraInfo))
                        {
                            ...
                            if (!pActiveCameraEditorData->hasLoadedSettings)
                            {
                                ...
                                if (iter != Settings::settingsType.cameraPresets.end())
                                {
                                    ...
                                    *pActiveCameraInfo = pActiveCameraPreset->InfoPreset.
                                                                              getGameInternalsCompliantData();
                                    ...
                                    pActiveCameraEditorData->nitrousFOVWidening = pActiveCameraPreset->
                                        NitrousFOVWidening;
                            ...
                            InternalVariables::setVariable(InternalVariables::nosFOVWidening,
                                                           pActiveCameraEditorData->nitrousFOVWidening);
                        }
                    }
                    ...
                    if (pActiveCameraInfo && pActiveCameraEditorData)
                    {
                        ...
                        if (pActiveCameraEditorData->speedFOVEnabled)
                        {
                            ...
                            if (playerCarSpeed > 10.0f)
                                mod0 = min(pActiveCameraEditorData->speedFOVScale,
                                           (playerCarSpeed / 360.0f) * pActiveCameraEditorData->speedFOVScale);
            ...
            const virtual bool displayMenu() override
            {
                ...
                if (pActiveCameraIndex)
                {
                    ImGui::TextWrapped("Camera Index");
                    ImGui::SameLine();
                    ...
                    if (pActiveCameraInfo)
                    {
                        ...
                        if (ImGui::Checkbox("Controller View", &pActiveCameraEditorData->joyViewEnabled))
                        {
                            if (pActiveCameraEditorData->joyViewEnabled)
                            {
                                ...
                                {
                                    memcpy_s(pActiveCameraEditorData->defaultCameraInfo.Angle, sizeof(float) * 2,
                                             pActiveCameraInfo->Angle, sizeof(float) * 2);
                                    memcpy_s(pActiveCameraEditorData->defaultCameraInfo.Lag, sizeof(float) * 2,
                                             pActiveCameraInfo->Lag, sizeof(float) * 2);
                                }
                        ...
                        if (ImGui::Checkbox("Speed FOV", &pActiveCameraEditorData->speedFOVEnabled))
                        {
                            if (pActiveCameraEditorData->speedFOVEnabled)
                            {
                                ...
                                {
                                    memcpy_s(pActiveCameraEditorData->defaultCameraInfo.FOV, sizeof(float) * 2,
                                             pActiveCameraInfo->FOV, sizeof(float) * 2);
                                }
                            }
                            else
                        ...
                        if (pActiveCameraEditorData->speedFOVEnabled)
                            ImGui::SliderFloat("##SpeedFOVScale", &pActiveCameraEditorData->speedFOVScale, -50.0f,
                                               50.0f, "Speed FOV Scale: %.1f");

                        if (ImGui::SliderFloat("##NitrousFOVScale",
                                               pActiveCameraEditorData->getNitrousFOVWideningAsPScale(), -30.0f, 30.0f,
                                               "NOS FOV Scale: %.1f"))
                        ...
                        {
                            auto* pCameraPreset = &Settings::settingsType.cameraPresets[std::string(
                                pActiveCameraInfo->CollectionName)];
                            ...
                            pCameraPreset->NitrousFOVWidening = InternalVariables::getVariable(
                                InternalVariables::nosFOVWidening);
                        ...
                        if (!pActiveCameraEditorData->joyViewEnabled)
                        {
                            ...
                            ImGui::SliderFloat("##CameraHorizontalAngle", &pActiveCameraInfo->Angle[0], -45.0f, 45.0f,
                                               "%.3f deg");
                            if (pActiveCameraEditorData->showAdvancedOptions)
                            {
                                ImGui::Text("ANGLE[1]");
                                ImGui::SameLine();
                                ImGui::SliderFloat("##CameraHorizontalAngle_1", &pActiveCameraInfo->Angle[1], -45.0f,
                                                   45.0f, "%.3f deg");
                            }
                        ...
                        else
                        {
                            ...
                            ImGui::TextWrapped(
                                "Cannot change horizontal angle or distance while 'Controller View' is enabled!");
                            ImGui::PopStyleColor();
                        }
                        ...
                        ImGui::SliderFloat("##CameraLateOffset", &pActiveCameraInfo->LateOffset[0], -89.0f, 89.0f,
                                           "%.3f deg");
                        if (pActiveCameraEditorData->showAdvancedOptions)
                        {
                            ImGui::Text("LATEOFFSET[1]");
                            ImGui::SameLine();
                            ImGui::SliderFloat("##CameraLateOffset_1", &pActiveCameraInfo->LateOffset[1], -89.0f,
                                               89.0f);
                        }
