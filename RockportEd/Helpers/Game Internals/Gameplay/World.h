...
namespace GameInternals
{
    struct TimeOfDayLightingData
    {
        ...
        void operator=(const TimeOfDayLightingData& newTimeOfDayLightingData)
        {
            ...
            memcpy_s(FixedFunctionSkyColour, sizeof(float) * 4, newTimeOfDayLightingData.FixedFunctionSkyColour,
                     sizeof(float) * 4);
    ...
    struct TimeOfDay
    {
        ...
        /* struct align: Floats related to something, maybe the result of the lighting data mixing?? */
        unsigned char __unk[0x4 * 44];
    ...
    namespace Gameplay
    {
        namespace World
        {
            ...
            struct TimeOfDayLightingBackup
            {
                ...
                TimeOfDayLightingBackup(const TimeOfDayLighting* _timeOfDayLighting,
                                        const TimeOfDayLightingData* _timeOfDayLightingData)
                {
            ...
            static bool loadLightingDefinitions()
            {
                ...
                {
                    TimeOfDayLightingDefinition** pArrTODLighting = (TimeOfDayLightingDefinition**)Memory::readPointer(
                        0x5B392C, false, 3, 0xEC, 0x14, 0x1C);
                    if (!pArrTODLighting)
                        return false;
                    ...
                    for (;;)
                    {
                        ...
                        {
                            backups.push_back(new TimeOfDayLightingBackup(
                                arrTODLighting->pTimeOfDayLighting, arrTODLighting->pTimeOfDayLighting->pLightingData));
                                    ...
                                    TimeOfDayLighting* out_pTODLightingMixResult)
            {
                ...
                *out_pTODLightingMixResult->pLightingData = (*pTODLighting_1->pLightingData * intensity_1) + (*
                    pTODLighting_2->pLightingData * intensity_2);
                out_pTODLightingMixResult->FogInLightScatter = (pTODLighting_1->FogInLightScatter * intensity_1) + (
                    pTODLighting_2->FogInLightScatter * intensity_2);
                out_pTODLightingMixResult->FogSunFalloff = (pTODLighting_1->FogSunFalloff * intensity_1) + (
                    pTODLighting_2->FogSunFalloff * intensity_2);
            }
